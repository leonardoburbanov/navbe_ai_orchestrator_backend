...
def read_execution(
    execution_id: int, service: ExecutionService = Depends(get_execution_service)
):
...
def read_all_executions(
    limit: int = 20, service: ExecutionService = Depends(get_execution_service)
):
    """
...
def read_process_executions(
    process_id: int, service: ExecutionService = Depends(get_execution_service)
):
